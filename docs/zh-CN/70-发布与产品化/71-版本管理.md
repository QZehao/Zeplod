> 语言: **中文** | [English](../../en/70-release-productization/71-version-management.md)
...
版本宏在构建时通过 `CMakeLists.txt` 中的 `target_compile_definitions`（`-D`）注入，
源头为仓库根目录的 `APP_VERSION` 文件：

- `PROJECT_VERSION_MAJOR/MINOR/PATCH`、`PROJECT_VERSION` —— 应用版本（来自 `APP_VERSION`）
- `GIT_COMMIT_HASH`、`GIT_BRANCH`、`GIT_TAG`、`GIT_DIRTY` —— Git 信息
- `BUILD_TIMESTAMP`、`BUILD_DATE`、`BUILD_TIME`、`BUILD_TARGET` —— 构建信息

`app_version.h` 使用上述宏（带 `#ifndef` 安全兜底值）。
...
| 文件 | 描述 |
|------|------|
| `APP_VERSION` | 版本号唯一源头（X.Y.Z） |
| `include/zeplod/app_version.h` | 版本 API 头文件 |
| `src/app/app_version.c` | 版本 API 实现 |
| `CMakeLists.txt` | 读取 `APP_VERSION`，通过 `-D` 注入版本宏 |
