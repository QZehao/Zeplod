...
| Join 超时 | 500 ms（`EVENT_DISPATCHER_THREAD_JOIN_TIMEOUT_MS`） | 500 ms（`IPC_SERVICE_THREAD_JOIN_TIMEOUT_MS`） | **无**（`K_FOREVER`） |
| Join 失败 | 返回 `EVENT_ERR_TIMEOUT`，**不** abort | abort 后重试 join；失败 → `ZEP_STATE_ERROR`，返回 `-EIO` | 阻塞至退出 |
...
- Data Bus join 无超时/abort，与 IPC/Event 不一致。
- IPC 使用 `abort`+重试 join；Event Dispatcher join 超时后保留 `thread_started` 以便重试。
...
| data_bus | `test_init_deinit`、`test_repeat_deinit_idempotent` |
