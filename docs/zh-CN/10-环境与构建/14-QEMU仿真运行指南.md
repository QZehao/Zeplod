...
### 7.2 `west build -t run` 在非交互环境失败

QEMU 默认将串口绑定到 **stdio**。请在本地 PowerShell / 终端窗口中运行，不要依赖无 TTY 的后台任务。

### 7.3 `qemu_cortex_m3` 上 HardFault / Lockup
