> 语言: **中文** | [English](../../en/40-app-development/44-devicetree-memory-config.md)

> **本文档范围**：Devicetree overlay 合并规则、`zephyr,sram`、多块内存、32KB 极限优化与 `zeplod_app_overlays.cmake` 行为。  
> **另见**：[12-Freestanding应用与构建基础](../10-环境与构建/12-Freestanding应用与构建基础.md)（BOARD_ROOT 概要）· [15-新建APP开发指南](../10-环境与构建/15-新建APP开发指南.md)（APP overlay）· [43-配置方案对比指南](43-配置方案对比指南.md)（方案选型）

# 设备树与内存配置手册

本手册说明在本工程（`zeplod`）及 Zephyr 通用机制下，**Devicetree Overlay 如何生效**、**主 RAM（`zephyr,sram`）如何影响链接与堆**，以及**多块、不连续内存**在 Zephyr 中的典型处理方式。适用于 Zephyr 4.x，构建系统以 CMake 生成链接脚本为准。

**阅读提示**：若尚未理解 **`west build`**、`prj.conf` 与 **`ZEPHYR_BASE`**，请先阅读 **[文档索引.md](../00-入门/02-文档索引.md)** 与 **[Freestanding应用与构建基础.md](../10-环境与构建/12-Freestanding应用与构建基础.md)**；本手册偏板级与链接细节，可按需查阅章节。
...
### 6.3 与 `conf/profiles/balanced.conf` 的关系

- **`conf/profiles/balanced.conf`** 为**说明性**片段（注释），可配合 **`prj.conf`** 使用；**Heap 大小**等仍由 **`CONFIG_HEAP_MEM_POOL_SIZE`** 等在 **`prj.conf`** 中配置。
...
### 11.0 示例 0：32KB SRAM 极限优化（conf/profiles/tiny.conf）

**场景**：MCU 仅 32KB SRAM，需要预留 22KB 给 APP 模块，框架需控制在 10KB 以内。

**`conf/profiles/tiny.conf`**（极限配置）：
...
### 11.2.1 单元测试应用 `tests/`（与主应用分离）

`west build -b nucleo_l4r5zi tests/` 时 **应用根目录为 `tests/`**，根目录 **`boards/nucleo_l4r5zi.overlay`** **不会**自动合并。本仓库提供：

- **`tests/boards/nucleo_l4r5zi.overlay`**（与上节 SRAM 节点相同）
- **`tests/CMakeLists.txt`** 在 `BOARD=nucleo_l4r5zi` 时追加该 overlay 到 **`DTC_OVERLAY_FILE`**

构建后请确认 **`RAM` Region Size = 640 KB**。详见 [硬件测试运行指南.md](../50-测试与CI/53-硬件测试运行指南.md) §6.4。
...
### 11.7 示例 G：使用说明性片段 `conf/profiles/balanced.conf`

本仓库 **`conf/profiles/balanced.conf`** 为**注释型**说明文件，**不会**被 Zephyr 自动加载。若要把其中的 heap/栈建议应用到构建，需：
