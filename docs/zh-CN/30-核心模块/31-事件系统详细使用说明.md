# 事件系统详细使用说明

## 目录

- [概述](#概述)
- [核心概念](#核心概念)
- [系统架构](#系统架构)
- [内存与所有权](#内存与所有权)
- [配置选项](#配置选项)
- [API 参考](#api-参考)
- [使用指南](#使用指南)
- [最佳实践](#最佳实践)
- [故障排除](#故障排除)

---

## 概述

事件系统是 Zephyr RTOS 框架的核心组件，提供高性能、线程安全的发布 - 订阅（Publish-Subscribe）模式通信机制。该系统支持：

- **异步事件处理**：解耦事件生产者与消费者
- **优先级调度**：支持事件优先级分类
- **线程安全**：所有操作均经过互斥锁保护
- **动态订阅**：运行时动态注册/注销事件类型和订阅者
- **ISR 支持**：支持从中断上下文发布事件

### 主要特性

| 特性 | 描述 |
|------|------|
| 最大事件类型数 | 256 |
| 每类型最大订阅者 | `CONFIG_EVENT_MAX_SUBSCRIBERS` (默认 8) |
| 事件队列大小 | 32 (可配置) |
| 调度器优先级 | 5 (可配置) |
| ISR 安全发布 | 支持 |
| **Slab 内存池** | 优先级分层 + 大数据块池 |
| **实时安全 API** | `_rt` 后缀，O(1) 确定分配时间 |
| **分发器** | 支持暂停/恢复、事件过滤、手动批处理 |

---

## 核心概念

### 事件（Event）

事件是系统中最基本的通信单元，包含以下字段：

```c
typedef struct {
    uint8_t          type;           // 事件类型标识符
    uint8_t          priority;       // 事件优先级
    uint8_t          flags;          // 标志位 (EVENT_FLAG_*)
    uint8_t          reserved;       // 预留扩展
    uint32_t         timestamp;      // 事件创建时间戳（毫秒 uptime）
    uint32_t         source_id;      // 源模块/组件 ID
    uint32_t         data_len;       // 事件数据长度
    union {
        uint8_t  inline_data[CONFIG_EVENT_INLINE_DATA_SIZE]; // 内联数据
        void*    ptr;                                             // 外部数据指针
    } data;
} event_t;
```

**内存布局**（以 64 字节配置为例）：
```
┌────────────────────────────────┐
│ type(1) priority(1) flags(1) ? │  4B
│ timestamp                      │  4B
│ source_id                      │  4B
│ data_len                       │  4B
├────────────────────────────────┤  16B 头部
│ inline_data[48] 或 ptr(8)      │ 48B
└────────────────────────────────┘  64B 总计
```

**数据存储策略**：
- `data_len ≤ CONFIG_EVENT_INLINE_DATA_SIZE`：内联存储，无额外分配
- `data_len > CONFIG_EVENT_INLINE_DATA_SIZE`：从 Slab 池或 k_malloc 分配

**标志位定义**：
| 标志 | 值 | 含义 |
|------|-----|------|
| `EVENT_FLAG_DATA_INLINE` | 0x01 | 数据内联存储于 inline_data |
| `EVENT_FLAG_DATA_DYNAMIC` | 0x02 | 数据动态分配，由系统管理生命周期 |
| `EVENT_FLAG_FROM_SLAB` | 0x04 | event_t 来自 Slab 池 |
| `EVENT_FLAG_DATA_FROM_SLAB` | 0x08 | 数据来自 Slab 池（随事件释放） |
| `EVENT_FLAG_SLAB_256` | 0x10 | 当前数据落在 256B slab 池 |
| `EVENT_FLAG_SLAB_1K` | 0x20 | 当前数据落在 1KB slab 池 |
| `EVENT_FLAG_SLAB_4K` | 0x40 | 当前数据落在 4KB slab 池 |
| `EVENT_FLAG_SLAB_MASK` | 0x70 | 覆盖三个 slab 大小位的掩码 |

### 事件类型（Event Type）

事件类型用于分类事件，系统支持最多 256 种事件类型。预定义的通用事件类型：

```c
#define EVENT_TYPE_GENERIC        1U    // 通用事件
#define EVENT_TYPE_SENSOR_DATA    10U   // 传感器数据事件
#define EVENT_TYPE_SENSOR_CONFIG  11U   // 传感器配置事件
```

### 订阅者（Subscriber）

订阅者是注册接收特定类型事件的回调函数：

```c
typedef struct {
    event_callback_t callback;      // 回调函数
    void            *user_data;     // 用户数据
    uint32_t         subscriber_id; // 唯一订阅者 ID
    bool             is_active;     // 是否激活
} subscriber_entry_t;
```

### 事件优先级

```c
typedef enum {
    EVENT_PRIORITY_LOW      = 10,   // 低优先级
    EVENT_PRIORITY_NORMAL   = 5,    // 普通优先级
    EVENT_PRIORITY_HIGH     = 2,    // 高优先级
    EVENT_PRIORITY_CRITICAL = 0     // 关键优先级
} event_priority_t;
```

---

## 系统架构

```
┌─────────────────────────────────────────────────────────────┐
│                      应用层 (Application)                    │
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │  Module A   │  │  Module B   │  │  Module C   │         │
│  │ (Publisher) │  │(Subscriber) │  │(Subscriber) │         │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘         │
│         │                │                │                 │
│         │ event_publish  │                │                 │
│         ▼                │                │                 │
│  ┌─────────────────────────────────────────────────────┐   │
│  │              事件系统核心 (Event System)              │   │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  │   │
│  │  │ Event Queue │  │Dispatcher   │  │ Type Mgr    │  │   │
│  │  │ (k_msgq)    │  │ (Thread)    │  │ (Registry)  │  │   │
│  │  └─────────────┘  └─────────────┘  └─────────────┘  │   │
│  └─────────────────────────────────────────────────────┘   │
│         │                │                │                 │
│         │ event_notify   │                │                 │
│         ▼                ▼                ▼                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │  Callback A │  │  Callback B │  │  Callback C │         │
│  └─────────────┘  └─────────────┘  └─────────────┘         │
└─────────────────────────────────────────────────────────────┘
```

### 组件说明

1. **事件队列（Event Queue）**
   - 基于 Zephyr `k_msgq` 实现
   - 支持优先级排队
   - 溢出时丢弃新事件

2. **事件调度器（Event Dispatcher）**
   - 独立线程运行
   - 从队列取出事件并通知订阅者
   - 可配置优先级和栈大小

3. **事件类型管理器（Event Type Manager）**
   - 维护事件类型注册表
   - 管理每个类型的订阅者列表
   - 线程安全的访问控制

---

## 内存与所有权

本节说明事件在 **入队、分发、回调** 各阶段由谁分配、谁释放，避免误用导致 **use-after-free、双重释放或内存泄漏**。

### Slab 内存管理概述

事件系统支持两种内存分配模式：

| 模式 | API | 内存来源 | 实时安全 | 适用场景 |
|------|-----|----------|----------|----------|
| **Slab 模式** | `event_create_rt`, `event_create_with_data_rt` | 预分配 Slab 池 | ✅ O(1) 确定 | ISR、实时任务 |
| **堆模式** | `event_create`, `event_create_with_data` | k_malloc / Slab 回退 | ⚠️ 可能阻塞 | 普通任务 |

**Slab 池架构**：

```
┌─────────────────────────────────────────────────────────────┐
│                    事件 Slab 池 (event_t)                     │
├───────────────┬───────────────┬───────────────────────────┤
│  CRITICAL 池  │   HIGH 池     │      NORMAL/LOW 池        │
│   (可选)      │   (可选)      │        (必须)             │
├───────────────┴───────────────┴───────────────────────────┤
│                    数据 Slab 池 (可选)                       │
├─────────────┬─────────────┬─────────────────────────────┤
│   256B 池   │    1KB 池   │         4KB 池             │
└─────────────┴─────────────┴─────────────────────────────┘
```

**数据存储策略**：
- `data_len ≤ CONFIG_EVENT_INLINE_DATA_SIZE`（默认 48B）：内联存储，零额外分配
- `data_len ≤ 256`：256B 数据池
- `data_len ≤ 1024`：1KB 数据池
- `data_len ≤ 4096`：4KB 数据池
- `data_len > 4096`：回退到 k_malloc（非实时安全）

### 级联 Fallback 机制

`event_memory_select_data_slab_with_fallback` 函数实现了**级联回退**策略：

1. 首选：根据 `data_len` 选择最小合适的 Slab 池（256B → 1KB → 4KB）
2. 检查：如果首选 Slab 池有可用块，直接返回
3. 回退：如果首选池已满，尝试更大的 Slab 池
4. 最终：如果所有 Slab 池都满，回退到 `k_malloc`

**内存放大代价**：

级联回退可能导致"内存放大"——例如 300 字节的数据最终使用 4KB Slab 块。这是因为：
- Slab 块大小是固定的，无法分割
- `EVENT_FLAG_SLAB_*` 标志记录了实际分配来源，确保释放时配对正确

**为什么要用 `EVENT_FLAG_SLAB_*` 标志**：

当数据通过级联 fallback 分配到比实际需求更大的 Slab 时，释放必须回退到正确的 Slab 池。例如：
- 300B 数据首选 256B 池（满）→ 分配到 1KB 池 → 设置 `EVENT_FLAG_SLAB_1K`
- 释放时检查 `EVENT_FLAG_SLAB_*`，将块归还给 1KB 池而非 256B 池

**实时安全注意事项**：

级联 fallback 发生在 `event_create_with_data` 等**非实时**路径。实时安全 API（`_rt` 后缀）不使用 fallback，内存不足时直接返回错误。

### 队列里到底存了什么

事件队列基于 Zephyr `k_msgq` 初始化，单条消息大小为 `sizeof(event_t)`（可配置 32/64/128 字节）：

- `k_msgq_put` 会把调用方传入指针所指的 **`event_t` 整结构体按字节拷贝**到队列环形缓冲区中，**不是**只存「指向 `event_t` 的指针」。
- 结构体里的 `data` 成员是 **联合体**：
  - 小数据（≤ INLINE_DATA_SIZE）：存储在 `inline_data[48]` 中，完整复制
  - 大数据：`ptr` 指针被复制，指向的堆/Slab 内存不复制

分发线程 `k_msgq_get` 时，会把队列槽位里的 `event_t` **再拷贝一份到线程栈上的局部变量**，然后调用 `event_notify_subscribers`，最后在适当时机释放动态分配的数据（见下）。

### `event_publish_copy`（推荐）：模块无需手动释放

流程概要：

1. `event_create_with_data` 分配 **`event_t` 外壳**（优先 Slab，回退 k_malloc）与 **数据缓冲区**（小数据内联，大数据 Slab/k_malloc），并把调用方数据 `memcpy` 进缓冲区。
2. `k_msgq_put` 将 **`event_t` 的值** 写入队列；队列中的副本携带完整事件数据。
3. 入队成功后，实现清除 `EVENT_FLAG_DATA_DYNAMIC` 标志，再 **释放 `event_t` 外壳**（Slab 或 k_free）。
4. 分发线程取出事件、执行回调后，若 `flags & EVENT_FLAG_DATA_DYNAMIC && data.ptr != NULL`，则释放数据内存。

**结论**：走 `event_publish_copy` 时，模块 **不需要** 也 **不应该** 再释放本次事件的 `data` 或 `event_t`；发布失败时由 `event_publish_copy` 内部统一清理。

### `event_publish_copy_rt`（实时安全）：Slab 模式推荐

```c
// ✅ 推荐：ISR 和实时关键任务使用 _rt 后缀 API
void isr_handler(void)
{
    uint8_t data[16] = {0x01, 0x02};
    
    event_status_t ret = event_publish_copy_rt(
        EVENT_TYPE_SENSOR_DATA,
        EVENT_PRIORITY_HIGH,
        data,
        sizeof(data)
    );
    
    if (ret == EVENT_ERR_NO_MEM) {
        // Slab 耗尽，可采取降级措施
    }
}
```

**特点**：
- 完全从 Slab 池分配，O(1) 确定时间
- Slab 耗尽时返回 NULL/错误，**不回退** k_malloc
- 小数据（≤ INLINE_DATA_SIZE）内联存储，零额外分配

### `event_publish`：入队成功后的所有权转移

`event_publish` / `event_publish_from_isr` 将 **`event_t` 按值拷贝** 进 `k_msgq`（见上文「队列里到底存了什么」）。

- **入队成功**：实现会清除调用方 `event` 上的 `EVENT_FLAG_DATA_DYNAMIC` 等动态数据标记，表示 **动态负载已交给队列内副本**；调用方 **不得** 再 `event_free_data()`，但 **可以** 对同一指针调用 `event_free()` 释放 Slab/堆上的 **`event_t` 外壳**（栈上局部变量无需释放）。
- **入队失败**（如 `EVENT_ERR_QUEUE_FULL`）：调用方 **仍拥有** 动态数据，须自行 `event_free_data()` / `event_free()`。

分发线程在处理队列副本时释放动态 `data.ptr`；**不会** 释放你当初 `event_create` 得到的那颗外壳（成功入队后应由调用方 `event_free`）。

因此分情形说明：

| 发布方式 | `event_t` 来源 | 数据存储 | 谁释放 `data` | 谁释放 `event_t` 外壳 |
|----------|---------------|----------|---------------|-------------------------|
| `event_publish_copy` | 内部临时 | 内联/Slab/堆 | 分发线程 | 函数内部 |
| `event_publish_copy_rt` | 内部临时 Slab | 内联/Slab | 分发线程 | 函数内部 |
| `event_publish` + 栈上局部 `event_t` | 栈 | 内联 | 分发线程（副本） | 栈自动结束 |
| `event_publish` + `event_create*` 堆对象 | Slab/堆 | 内联/Slab/堆 | 分发线程（副本） | **入队成功后调用方 `event_free`** |

**实践建议**：

- **普通任务**：优先使用 `event_publish_copy`
- **ISR/实时任务**：使用 `event_publish_copy_rt`
- **栈上小数据**：可使用 `event_publish` + 栈上 `event_t`（数据内联）

### `event_publish_from_isr`

- ISR 中 **推荐** 使用 `event_publish_copy_rt` 或 `event_create_from_isr` + `event_publish`
- 使用实时安全 API 可保证 O(1) 确定时间，避免堆分配的不可预测性
- **技术细节**：`k_msgq_put(K_NO_WAIT)` 在 ISR 中安全，因为 Zephyr `k_msgq` 内部使用 **spinlock**（自旋锁），而非 mutex（互斥锁）
  - **Mutex**：可能阻塞、触发线程调度，ISR 中禁止使用
  - **Spinlock**：只禁用本地中断、不会调度，ISR 中安全

### 订阅回调中的规则

- 回调收到的 `const event_t *event` 指向 **分发线程栈上的副本**，**仅在回调执行期间有效**
- **不要** 保存该指针供异步使用
- **不要** 在回调里 `k_free(event->data.ptr)`：动态数据由分发线程在回调全部返回后释放

### 与「最佳实践」中示例的对应关系

入队成功后：**不要** 再 `event_free_data()`，**不要** 修改已清除动态标记的 `event` 内容；堆上外壳请 **`event_free(event)`**。`event_publish_copy` 则在内部完成外壳与数据的完整生命周期管理。

---

## 配置选项

事件系统相关 Kconfig 项的**逐项释义**见 **[项目配置项说明.md](../40-应用开发/42-项目配置项说明.md) §1**。常用配置示例：

```kconfig
CONFIG_EVENT_SYSTEM=y
CONFIG_EVENT_QUEUE_SIZE=64
CONFIG_EVENT_MAX_SUBSCRIBERS=8
CONFIG_EVENT_DISPATCHER_STACK_SIZE=2048
CONFIG_EVENT_DISPATCHER_PRIORITY=5
CONFIG_EVENT_STRUCT_SIZE_64=y
CONFIG_EVENT_INLINE_DATA_SIZE=48
```

### Slab 池配置

```kconfig
CONFIG_EVENT_SLAB_ENABLE=y
CONFIG_EVENT_SLAB_CRITICAL_COUNT=8
CONFIG_EVENT_SLAB_HIGH_COUNT=16
CONFIG_EVENT_SLAB_NORMAL_COUNT=32
CONFIG_EVENT_SLAB_LARGE_ENABLE=y
CONFIG_EVENT_SLAB_LARGE_256_COUNT=8
CONFIG_EVENT_SLAB_LARGE_1K_COUNT=4
CONFIG_EVENT_SLAB_LARGE_4K_COUNT=2
```

### 运行时调试配置

```kconfig
CONFIG_EVENT_RUNTIME_STATUS=y
CONFIG_EVENT_DEBUG_MEM=y
CONFIG_EVENT_SLAB_EXHAUSTED_CB=y
CONFIG_EVENT_QUEUE_HIGH_WATERMARK=48
CONFIG_EVENT_DISPATCHER_MAX_EVENTS_PER_CYCLE=100
```

---

## API 参考

### 核心 API

#### 初始化与启动

```c
/**
 * @brief 初始化事件系统
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_system_init(void);

/**
 * @brief 启动事件调度器
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_system_start(void);

/**
 * @brief 停止事件调度器
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_system_stop(void);

/**
 * @brief 检查事件系统是否运行
 * @return true 运行中，false 已停止
 */
bool event_system_is_running(void);

/**
 * @brief 关闭事件系统
 * @return EVENT_OK 成功，其他为错误码
 * @note 停止分发器线程，释放所有资源
 */
event_status_t event_system_shutdown(void);

/**
 * @brief 重置事件系统统计信息
 */
void event_system_reset_statistics(void);
```

#### 事件类型管理

```c
/**
 * @brief 注册事件类型
 * @param type 事件类型 ID (0-255)
 * @param name 事件类型名称
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_register_type(event_type_t type, const char *name);

/**
 * @brief 注销事件类型
 * @param type 事件类型 ID
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_unregister_type(event_type_t type);
```

#### 订阅管理

```c
/**
 * @brief 订阅事件类型
 * @param type 事件类型 ID
 * @param callback 回调函数
 * @param user_data 用户数据（传递给回调）
 * @param subscriber_id 输出：分配的订阅者 ID
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_subscribe(event_type_t type,
                                event_callback_t callback,
                                void *user_data,
                                uint32_t *subscriber_id);

/**
 * @brief 取消订阅特定事件类型
 * @param type 事件类型 ID
 * @param subscriber_id 订阅者 ID
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_unsubscribe(event_type_t type, uint32_t subscriber_id);

/**
 * @brief 取消订阅所有事件类型
 * @param subscriber_id 订阅者 ID
 */
void event_unsubscribe_all(uint32_t subscriber_id);
```

#### 事件发布

```c
/**
 * @brief 发布事件（同步）
 * @param event 事件指针
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_publish(event_t *event);

/**
 * @brief 从中断上下文发布事件
 * @param event 事件指针（所有权语义同 event_publish）
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_publish_from_isr(event_t *event);

/**
 * @brief 发布事件并复制数据（内部创建副本）
 * @param type 事件类型 ID
 * @param priority 事件优先级
 * @param data 数据指针
 * @param data_len 数据长度
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_publish_copy(event_type_t type,
                                   event_priority_t priority,
                                   const void *data,
                                   size_t data_len);
```

#### 事件创建与内存管理

```c
/**
 * @brief 创建空事件
 * @param type 事件类型 ID
 * @param priority 事件优先级
 * @return 事件指针，失败返回 NULL
 */
event_t *event_create(event_type_t type, event_priority_t priority);

/**
 * @brief 创建带数据的事件
 * @param type 事件类型 ID
 * @param priority 事件优先级
 * @param data 数据指针
 * @param data_len 数据长度
 * @return 事件指针，失败返回 NULL
 */
event_t *event_create_with_data(event_type_t type,
                                 event_priority_t priority,
                                 const void *data,
                                 size_t data_len);

/**
 * @brief 释放事件
 * @param event 事件指针
 */
void event_free(event_t *event);
```

#### 实时安全 API（Slab 池）

以下 API 完全从 Slab 池分配内存，分配时间 O(1) 确定，适合 ISR 和实时关键任务：

```c
/**
 * @brief 创建事件（实时安全）
 * @param type 事件类型 ID
 * @param priority 事件优先级
 * @return 指向新事件的指针，失败返回 NULL
 * @note 完全从 Slab 池分配，不回退 k_malloc
 */
event_t *event_create_rt(event_type_t type, event_priority_t priority);

/**
 * @brief 创建带数据的事件（实时安全）
 * @param type 事件类型 ID
 * @param priority 事件优先级
 * @param data 数据指针
 * @param data_len 数据长度
 * @return 指向新事件的指针，失败返回 NULL
 * @note 小数据内联存储；大数据从 Slab 分配，不回退 k_malloc
 */
event_t *event_create_with_data_rt(event_type_t type,
                                    event_priority_t priority,
                                    const void *data,
                                    size_t data_len);

/**
 * @brief 发布事件并复制数据（实时安全）
 * @param type 事件类型 ID
 * @param priority 事件优先级
 * @param data 数据指针
 * @param data_len 数据长度
 * @return EVENT_OK 成功，其他错误码
 * @note 内存不足时返回错误，不回退 k_malloc
 */
event_status_t event_publish_copy_rt(event_type_t type,
                                      event_priority_t priority,
                                      const void *data,
                                      size_t data_len);

/**
 * @brief 从 ISR 创建事件（实时安全）
 * @note 等同于 event_create_with_data_rt，明确 ISR 上下文使用
 */
static inline event_t *event_create_from_isr(event_type_t type,
                                              event_priority_t priority,
                                              const void *data,
                                              size_t data_len);
```

#### 运行时状态查询（Slab 池）

```c
/**
 * @brief 检查指定优先级的 Slab 是否可用
 * @param priority 事件优先级
 * @return true 有可用块，false Slab 已耗尽或不可用
 */
bool event_slab_available(event_priority_t priority);

/**
 * @brief 获取指定优先级 Slab 的剩余块数
 * @param priority 事件优先级
 * @return 剩余可用块数，如果 Slab 不可用返回 0
 */
uint32_t event_slab_remaining(event_priority_t priority);

/**
 * @brief Slab 统计信息结构体
 */
typedef struct {
    uint32_t critical_used;     // CRITICAL 池已用块数
    uint32_t critical_total;    // CRITICAL 池总块数
    uint32_t high_used;         // HIGH 池已用块数
    uint32_t high_total;        // HIGH 池总块数
    uint32_t normal_used;       // NORMAL 池已用块数
    uint32_t normal_total;      // NORMAL 池总块数
    uint32_t data_256_used;     // 256B 数据池已用块数
    uint32_t data_256_total;    // 256B 数据池总块数
    uint32_t data_1k_used;      // 1KB 数据池已用块数
    uint32_t data_1k_total;     // 1KB 数据池总块数
    uint32_t data_4k_used;      // 4KB 数据池已用块数
    uint32_t data_4k_total;     // 4KB 数据池总块数
    uint32_t fallback_count;    // 回退到 k_malloc 的次数
} event_slab_stats_t;

/**
 * @brief 获取所有 Slab 池的统计信息
 * @param stats 输出参数，接收统计信息
 */
void event_get_slab_stats(event_slab_stats_t *stats);

/**
 * @brief 注册 Slab 耗尽回调
 * @param cb 回调函数，NULL 清除回调
 * @note 需要 CONFIG_EVENT_SLAB_EXHAUSTED_CB=y
 */
void event_register_slab_exhausted_cb(event_slab_exhausted_cb_t cb);

/**
 * @brief 检查内存泄漏
 * @return 未释放的分配数量
 * @note 需要 CONFIG_EVENT_DEBUG_MEM=y
 */
uint32_t event_check_leaks(void);

/**
 * @brief 打印内存泄漏详情
 * @note 需要 CONFIG_EVENT_DEBUG_MEM=y
 */
void event_dump_leaks(void);
```

#### 工具函数

```c
/**
 * @brief 获取事件类型名称
 * @param type 事件类型 ID
 * @return 类型名称字符串
 */
const char *event_get_type_name(event_type_t type);

/**
 * @brief 获取事件类型的订阅者数量
 * @param type 事件类型 ID
 * @return 订阅者数量
 */
uint32_t event_get_subscriber_count(event_type_t type);

/**
 * @brief 获取事件系统统计信息
 * @param total_events 输出：总事件数
 * @param queue_depth 输出：当前队列深度
 * @param dropped_events 输出：丢弃的事件数
 */
void event_get_statistics(uint32_t *total_events,
                          uint32_t *queue_depth,
                          uint32_t *dropped_events);

/**
 * @brief 获取全局事件队列
 * @return 事件队列指针
 */
struct k_msgq *event_system_get_queue(void);

/**
 * @brief 手动通知订阅者（用于自定义调度）
 * @param event 事件指针
 * @return EVENT_OK 成功，其他为错误码
 */
event_status_t event_notify_subscribers(const event_t *event);
```

### 事件队列 API

```c
/**
 * @brief 初始化事件队列
 */
event_status_t event_queue_init(struct k_msgq *queue, void *buffer,
                                 size_t capacity);

/**
 * @brief 入队事件
 */
event_status_t event_queue_enqueue(struct k_msgq *queue,
                                    const event_t *event,
                                    queue_overflow_policy_t policy,
                                    k_timeout_t timeout);

/**
 * @brief 出队事件
 */
event_status_t event_queue_dequeue(struct k_msgq *queue,
                                    event_t *event,
                                    k_timeout_t timeout);

/**
 * @brief 检查队列是否为空
 * @return true 空，false 非空
 */
bool event_queue_is_empty(const struct k_msgq *queue);

/**
 * @brief 检查队列是否已满
 * @return true 满，false 未满
 */
bool event_queue_is_full(const struct k_msgq *queue);

/**
 * @brief 获取队列深度（已用槽位数）
 * @return 队列中的事件数量
 */
uint32_t event_queue_depth(const struct k_msgq *queue);

/**
 * @brief 获取队列容量
 * @return 队列最大容量
 */
uint32_t event_queue_capacity(const struct k_msgq *queue);

/**
 * @brief 清空队列
 */
void event_queue_purge(struct k_msgq *queue);

/**
 * @brief 获取队列统计信息
 * @param stats 输出统计信息
 */
void event_queue_get_stats(const struct k_msgq *queue, queue_stats_t *stats);

/**
 * @brief 重置队列统计信息
 */
void event_queue_reset_stats(struct k_msgq *queue);

/**
 * @brief 反初始化事件队列（释放资源）
 */
void event_queue_deinit(struct k_msgq *queue);
```

### 事件调度器 API

```c
/**
 * @brief 初始化调度器
 * @param config 分发器配置，NULL 使用默认配置
 * @return EVENT_OK 成功
 */
event_status_t event_dispatcher_init(const dispatcher_config_t *config);

/**
 * @brief 启动分发器
 * @return EVENT_OK 成功
 */
event_status_t event_dispatcher_start(void);

/**
 * @brief 停止分发器
 * @return EVENT_OK 成功
 */
event_status_t event_dispatcher_stop(void);

/**
 * @brief 暂停事件处理
 */
event_status_t event_dispatcher_pause(void);

/**
 * @brief 恢复事件处理
 */
event_status_t event_dispatcher_resume(void);

/**
 * @brief 获取分发器当前状态
 * @return 分发器状态 (DISPATCHER_STOPPED/RUNNING/PAUSED)
 */
dispatcher_state_t event_dispatcher_get_state(void);

/**
 * @brief 检查当前线程是否为分发器线程
 * @return true 是分发器线程，false 不是
 */
bool event_dispatcher_is_current_thread(void);

/**
 * @brief 设置事件过滤器
 * @param filter 过滤函数，NULL 清除过滤
 * @param user_data 用户数据
 */
void event_dispatcher_set_filter(event_filter_t filter, void *user_data);

/**
 * @brief 清除事件过滤器
 */
void event_dispatcher_clear_filter(void);

/**
 * @brief 处理单个事件（手动分发模式）
 * @param timeout 等待超时时间
 * @return EVENT_OK 成功，EVENT_ERR_QUEUE_EMPTY 队列空
 */
event_status_t event_dispatcher_process_one(k_timeout_t timeout);

/**
 * @brief 处理所有待处理事件
 * @param max_events 最大处理数量，0 表示全部
 * @return 已处理的事件数量
 */
uint32_t event_dispatcher_process_all(uint32_t max_events);

/**
 * @brief 获取调度器统计
 */
void event_dispatcher_get_stats(dispatcher_stats_t *stats);

/**
 * @brief 重置调度器统计
 */
void event_dispatcher_reset_stats(void);

/**
 * @brief 获取距上次事件处理完成后的空闲时间（微秒）
 * @return 空闲时间（微秒）
 */
uint32_t event_dispatcher_get_current_latency(void);
```

---

## 使用指南

### 快速开始

#### 1. 初始化事件系统

```c
#include <zeplod/event_system.h>

void app_main(void)
{
    // 初始化事件系统
    event_status_t ret = event_system_init();
    if (ret != EVENT_OK) {
        LOG_ERR("Failed to initialize event system: %d", ret);
        return;
    }

    // 启动事件调度器
    ret = event_system_start();
    if (ret != EVENT_OK) {
        LOG_ERR("Failed to start event system: %d", ret);
        return;
    }

    LOG_INF("Event system started successfully");
}
```

#### 2. 注册事件类型

```c
// 定义自定义事件类型
#define MY_EVENT_TYPE_SENSOR_READ    100U
#define MY_EVENT_TYPE_DATA_READY     101U
#define MY_EVENT_TYPE_ALARM          102U

void register_event_types(void)
{
    event_register_type(MY_EVENT_TYPE_SENSOR_READ, "sensor_read");
    event_register_type(MY_EVENT_TYPE_DATA_READY, "data_ready");
    event_register_type(MY_EVENT_TYPE_ALARM, "alarm");
}
```

#### 3. 订阅事件

```c
static uint32_t g_subscriber_id;

// 事件回调函数
static void my_event_callback(const event_t *event, void *user_data)
{
    LOG_INF("Received event: type=%d, priority=%d, timestamp=%u",
            event->type, event->priority, event->timestamp);

    if (event->data_len > 0) {
        if (event->flags & EVENT_FLAG_DATA_INLINE) {
            uint8_t byte = event->data.inline_data[0];
            (void)byte;
        } else {
            uint8_t byte = *(uint8_t *)event->data.ptr;
            (void)byte;
        }
    }

    // 访问用户数据
    int *my_data = (int *)user_data;
    LOG_INF("User data: %d", *my_data);
}

void subscribe_to_events(void)
{
    int user_data = 42;

    event_status_t ret = event_subscribe(
        MY_EVENT_TYPE_SENSOR_READ,
        my_event_callback,
        &user_data,  // 用户数据指针
        &g_subscriber_id
    );

    if (ret != EVENT_OK) {
        LOG_ERR("Failed to subscribe: %d", ret);
    }
}
```

#### 4. 发布事件

```c
// 方法 1: 发布静态事件（无动态数据，栈上 event_t）
void publish_simple_event(void)
{
    event_t event = {
        .type = MY_EVENT_TYPE_SENSOR_READ,
        .priority = EVENT_PRIORITY_NORMAL,
        .timestamp = k_uptime_get_32(),
        .source_id = 1,
        .flags = EVENT_FLAG_DATA_INLINE,  // 内联数据，无额外分配
        .reserved = 0,  // 必须显式初始化
        .data_len = 0
    };

    event_publish(&event);
}

// 方法 2: 带 payload 时优先用 event_publish_copy（推荐）
// 系统会自动管理内存；模块无需 event_free。
void publish_data_event(const void *data, size_t len)
{
    event_publish_copy(
        MY_EVENT_TYPE_DATA_READY,
        EVENT_PRIORITY_HIGH,
        data,
        len
    );
}

// 方法 3: ISR 或实时任务使用 _rt 后缀 API（实时安全）
void isr_publish_event(void)
{
    uint8_t sensor_data[16] = {0x01, 0x02};

    // 完全从 Slab 池分配，O(1) 确定时间
    event_status_t ret = event_publish_copy_rt(
        MY_EVENT_TYPE_SENSOR_READ,
        EVENT_PRIORITY_HIGH,
        sensor_data,
        sizeof(sensor_data)
    );

    if (ret == EVENT_ERR_NO_MEM) {
        // Slab 池耗尽，采取降级措施
    }
}

// 方法 4: 手动创建和发布（不推荐，易泄漏）
void manual_publish(const void *data, size_t len)
{
    event_t *event = event_create_with_data(
        MY_EVENT_TYPE_SENSOR_READ,
        EVENT_PRIORITY_NORMAL,
        data,
        len
    );
    if (event == NULL) {
        return;
    }
    if (event_publish(event) != EVENT_OK) {
        event_free(event);  // 发布失败，释放整颗事件
    }
    // 发布成功时，data 由分发线程释放，但 event_t 外壳不会自动释放
    // 推荐改用 event_publish_copy，避免遗漏
}
```

#### 5. 取消订阅

```c
void unsubscribe_from_events(void)
{
    // 取消订阅特定事件
    event_unsubscribe(MY_EVENT_TYPE_SENSOR_READ, g_subscriber_id);

    // 或取消订阅所有事件
    // event_unsubscribe_all(g_subscriber_id);
}
```

### 完整示例

```c
#include <zephyr/kernel.h>
#include <zephyr/logging/log.h>
#include <zeplod/event_system.h>

LOG_MODULE_REGISTER(event_example, LOG_LEVEL_INF);

// 定义事件类型
#define EVENT_TYPE_BUTTON    50U
#define EVENT_TYPE_LED       51U

// 全局订阅者 ID
static uint32_t button_sub_id;
static uint32_t led_sub_id;

// 按钮事件回调
static void button_callback(const event_t *event, void *user_data)
{
    LOG_INF("Button event received: data_len=%u", event->data_len);
    
    if (event->data_len == sizeof(uint8_t)) {
        // 小数据内联存储，直接从 inline_data 读取
        uint8_t button_state;
        if (event->flags & EVENT_FLAG_DATA_INLINE) {
            button_state = event->data.inline_data[0];
        } else {
            button_state = *(uint8_t *)event->data.ptr;
        }
        LOG_INF("Button state: %s", button_state ? "PRESSED" : "RELEASED");
    }
}

// LED 事件回调
static void led_callback(const event_t *event, void *user_data)
{
    LOG_INF("LED event received");
    
    if (event->data_len == sizeof(uint8_t)) {
        uint8_t led_state;
        if (event->flags & EVENT_FLAG_DATA_INLINE) {
            led_state = event->data.inline_data[0];
        } else {
            led_state = *(uint8_t *)event->data.ptr;
        }
        // 控制 LED
        LOG_INF("LED turned %s", led_state ? "ON" : "OFF");
    }
}

// 按钮线程（发布按钮事件）
static void button_thread(void *p1, void *p2, void *p3)
{
    while (1) {
        uint8_t button_state = 1;  // 模拟按钮按下
        
        event_publish_copy(
            EVENT_TYPE_BUTTON,
            EVENT_PRIORITY_NORMAL,
            &button_state,
            sizeof(button_state)
        );

        k_msleep(1000);
        
        button_state = 0;  // 模拟按钮释放
        event_publish_copy(
            EVENT_TYPE_BUTTON,
            EVENT_PRIORITY_NORMAL,
            &button_state,
            sizeof(button_state)
        );

        k_msleep(1000);
    }
}

// LED 控制线程（发布 LED 事件）
static void led_thread(void *p1, void *p2, void *p3)
{
    k_sleep(K_SECONDS(2));  // 等待按钮线程先启动

    while (1) {
        uint8_t led_state = 1;  // LED ON
        
        event_publish_copy(
            EVENT_TYPE_LED,
            EVENT_PRIORITY_LOW,
            &led_state,
            sizeof(led_state)
        );

        k_msleep(500);
        
        led_state = 0;  // LED OFF
        event_publish_copy(
            EVENT_TYPE_LED,
            EVENT_PRIORITY_LOW,
            &led_state,
            sizeof(led_state)
        );

        k_msleep(500);
    }
}

// 主初始化函数
void event_system_example_init(void)
{
    // 1. 初始化事件系统
    event_system_init();

    // 2. 注册事件类型
    event_register_type(EVENT_TYPE_BUTTON, "button");
    event_register_type(EVENT_TYPE_LED, "led");

    // 3. 订阅事件
    event_subscribe(EVENT_TYPE_BUTTON, button_callback, NULL, &button_sub_id);
    event_subscribe(EVENT_TYPE_LED, led_callback, NULL, &led_sub_id);

    // 4. 启动事件调度器
    event_system_start();

    // 5. 创建工作线程
    K_THREAD_DEFINE(button_tid, 1024, button_thread, NULL, NULL, NULL,
                    5, 0, K_NO_WAIT);
    K_THREAD_DEFINE(led_tid, 1024, led_thread, NULL, NULL, NULL,
                    6, 0, K_NO_WAIT);

    LOG_INF("Event system example initialized");
}
```

### ISR 中使用事件系统

```c
#include <zeplod/event_system.h>

// 方法 1: 使用 event_publish_copy_rt（推荐，实时安全）
static void gpio_callback_rt(const struct device *dev,
                              struct gpio_callback *cb,
                              uint32_t pins)
{
    uint8_t button_state = 1;

    // 完全从 Slab 池分配，O(1) 确定时间
    event_status_t ret = event_publish_copy_rt(
        EVENT_TYPE_BUTTON,
        EVENT_PRIORITY_HIGH,
        &button_state,
        sizeof(button_state)
    );

    if (ret == EVENT_ERR_NO_MEM) {
        // Slab 池耗尽，记录日志（注意：LOG 可能在 ISR 中受限）
    }
}

// 方法 2: 使用栈上 event_t + 内联数据（适合无 payload）
static void gpio_callback_stack(const struct device *dev,
                                 struct gpio_callback *cb,
                                 uint32_t pins)
{
    // 栈上事件结构体，小数据内联存储
    event_t event = {
        .type = EVENT_TYPE_BUTTON,
        .priority = EVENT_PRIORITY_HIGH,
        .timestamp = k_uptime_get_32(),
        .source_id = 0,
        .flags = EVENT_FLAG_DATA_INLINE,
        .reserved = 0,
        .data_len = 0
    };

    event_publish_from_isr(&event);
}

// 方法 3: 使用 event_create_from_isr（手动控制生命周期）
static void gpio_callback_manual(const struct device *dev,
                                  struct gpio_callback *cb,
                                  uint32_t pins)
{
    uint8_t button_state = 1;

    // 创建事件（从 Slab 分配，实时安全）
    event_t *event = event_create_from_isr(
        EVENT_TYPE_BUTTON,
        EVENT_PRIORITY_HIGH,
        &button_state,
        sizeof(button_state)
    );

    if (event != NULL) {
        if (event_publish_from_isr(event) != EVENT_OK) {
            event_free(event);  // 发布失败，释放事件
        }
        // 发布成功后，event 需要在某处被释放（不推荐此方式）
    }
}
```

---

## 最佳实践

### 发布、卸载与手动消费契约

| 场景 | 要求 |
|------|------|
| **发布** | 先 `event_register_type`；优先 `event_publish_copy` / `event_create_with_data`。`event_publish` / `event_publish_from_isr` 会校验 `flags` 与 `data_len`，手搓不一致的 `event_t` 返回 `EVENT_ERR_INVALID_ARG` |
| **模块卸载** | `event_system_stop()` → `event_unsubscribe_all(subscriber_id)` → 再释放 `user_data`。`event_unsubscribe` 返回后，分发器仍可能执行已快照的旧回调 |
| **手动 `process_one` / `process_all`** | 仅在 **init 后、首次 `event_dispatcher_start()` 之前** 使用。曾 start 后再 stop，或分发器 `RUNNING` 时由其他线程调用，会返回 `EVENT_ERR_INVALID_ARG` |
| **`DROP_LOWEST` 溢出策略** | 仅线程侧 `event_publish` 会按优先级挤占队列槽位；`event_publish_from_isr` 在队列满时仍为 `DROP_NEWEST` |

### 1. 事件设计原则

```c
// ✅ 推荐：使用紧凑的数据结构
typedef struct {
    int16_t temperature;
    int16_t humidity;
    uint32_t timestamp;
} sensor_data_t;

void publish_sensor_data(sensor_data_t *data)
{
    event_publish_copy(
        EVENT_TYPE_SENSOR_DATA,
        EVENT_PRIORITY_NORMAL,
        data,
        sizeof(*data)
    );
}

// ❌ 避免：传递过大的数据
void publish_large_data(void *large_buffer, size_t size)  // size > 256 bytes
{
    // 考虑使用指针 + 所有权转移模式
}
```

### 2. 回调函数设计

```c
// ✅ 推荐：快速处理，异步执行耗时操作
static void fast_callback(const event_t *event, void *user_data)
{
    // 快速处理：复制数据，触发信号量等
    k_sem_give(&processing_sem);
}

// ❌ 避免：在回调中执行耗时操作
static void slow_callback(const event_t *event, void *user_data)
{
    k_msleep(1000);  // 阻塞调度器！
    // 复杂计算...
}
```

### 3. 内存管理

完整约定见上文 **[内存与所有权](#内存与所有权)**。此处仅列常用写法。

```c
// ✅ 推荐：event_publish_copy — 普通任务使用
void safe_publish(void)
{
    uint8_t data[16];
    event_publish_copy(EVENT_TYPE_GENERIC, EVENT_PRIORITY_NORMAL,
                       data, sizeof(data));
}

// ✅ 推荐：event_publish_copy_rt — ISR 和实时任务使用
void realtime_safe_publish(void)
{
    uint8_t data[16];
    event_status_t ret = event_publish_copy_rt(
        EVENT_TYPE_GENERIC, EVENT_PRIORITY_HIGH,
        data, sizeof(data)
    );
    if (ret == EVENT_ERR_NO_MEM) {
        // Slab 池耗尽，采取降级措施
    }
}

// ✅ 可行：栈上 event_t、小数据内联存储
void stack_publish(void)
{
    uint32_t counter = 42;
    event_t event = {
        .type = EVENT_TYPE_GENERIC,
        .priority = EVENT_PRIORITY_NORMAL,
        .timestamp = k_uptime_get_32(),
        .source_id = 0,
        .flags = EVENT_FLAG_DATA_INLINE,
        .data_len = sizeof(counter),
    };
    memcpy(event.data.inline_data, &counter, sizeof(counter));
    event_publish(&event);
}

// ⚠️ 避免手动管理：event_create_with_data + event_publish 易泄漏
// 推荐改用 event_publish_copy 或 event_publish_copy_rt
```

### 4. 错误处理

```c
void robust_publish(const void *data, size_t len)
{
    event_status_t ret = event_publish_copy(
        EVENT_TYPE_SENSOR_DATA,
        EVENT_PRIORITY_NORMAL,
        data,
        len
    );

    switch (ret) {
    case EVENT_OK:
        // 成功
        break;
    case EVENT_ERR_QUEUE_FULL:
        LOG_WRN("Event queue full, data dropped");
        break;
    case EVENT_ERR_INVALID_ARG:
        LOG_ERR("Invalid event arguments");
        break;
    case EVENT_ERR_NO_MEM:
        LOG_ERR("Memory allocation failed");
        break;
    default:
        LOG_ERR("Unknown error: %d", ret);
        break;
    }
}
```

### 5. 统计与调试

```c
void print_event_stats(void)
{
    uint32_t total, depth, dropped;
    event_get_statistics(&total, &depth, &dropped);

    LOG_INF("=== Event System Statistics ===");
    LOG_INF("Total events processed: %u", total);
    LOG_INF("Current queue depth: %u", depth);
    LOG_INF("Dropped events: %u", dropped);

    if (dropped > 0) {
        LOG_WRN("Consider increasing CONFIG_EVENT_QUEUE_SIZE");
    }
}

// Slab 池状态查询（需要 CONFIG_EVENT_RUNTIME_STATUS=y）
void print_slab_stats(void)
{
    event_slab_stats_t stats;
    event_get_slab_stats(&stats);

    LOG_INF("=== Slab Pool Statistics ===");
    LOG_INF("CRITICAL: %u/%u used", stats.critical_used, stats.critical_total);
    LOG_INF("HIGH:      %u/%u used", stats.high_used, stats.high_total);
    LOG_INF("NORMAL:    %u/%u used", stats.normal_used, stats.normal_total);
    LOG_INF("Data 256B: %u/%u used", stats.data_256_used, stats.data_256_total);
    LOG_INF("Data 1KB:  %u/%u used", stats.data_1k_used, stats.data_1k_total);
    LOG_INF("Data 4KB:  %u/%u used", stats.data_4k_used, stats.data_4k_total);
    LOG_INF("Fallback to k_malloc: %u", stats.fallback_count);

    // 检查特定优先级的 Slab 可用性
    if (!event_slab_available(EVENT_PRIORITY_HIGH)) {
        LOG_WRN("HIGH priority Slab exhausted!");
    }
}
```

---

## 故障排除

### 常见问题

#### 1. 事件未送达

**症状**：发布事件后回调未执行

**检查清单**：
- [ ] 事件类型是否已注册？
- [ ] 订阅者是否成功订阅？
- [ ] 事件调度器是否已启动？
- [ ] 回调函数是否正确实现？

```c
// 调试步骤
void debug_event_delivery(void)
{
    // 检查事件类型
    LOG_INF("Event type name: %s", event_get_type_name(MY_EVENT_TYPE));

    // 检查订阅者数量
    uint32_t count = event_get_subscriber_count(MY_EVENT_TYPE);
    LOG_INF("Subscriber count: %u", count);

    // 检查系统状态
    LOG_INF("System running: %s", 
            event_system_is_running() ? "yes" : "no");
}
```

#### 2. 队列溢出

**症状**：日志显示 "Event queue full, event dropped"

**解决方案**：
```kconfig
# 增加队列大小
CONFIG_EVENT_QUEUE_SIZE=128

# 或优化事件处理速度
# 减少回调中的处理时间
```

#### 3. 内存泄漏 / 野指针

**症状**：系统运行一段时间后内存不足，或随机崩溃

**检查点**：
- 优先使用 `event_publish_copy`；详见 [内存与所有权](#内存与所有权)
- `event_publish` + `event_create*`：分发线程**不会**释放 `event_t` 外壳，堆路径易泄漏；失败路径须 `event_free`
- **禁止** `event_publish` 的 `data` 指向发布函数内的局部栈缓冲（回调可能晚于函数返回）
- ISR 中避免堆分配；`data` 指向的内存在分发前须一直有效
- 回调内不要 `k_free(event->data)`（除非项目另有所有权约定）

#### 4. 死锁问题

**症状**：系统卡死，看门狗复位

**原因**：在事件回调中调用可能阻塞的 API

**解决方案**：
```c
// ❌ 错误示例
static void bad_callback(const event_t *event, void *user_data)
{
    k_mutex_lock(&some_mutex, K_FOREVER);  // 可能死锁
}

// ✅ 正确示例
static void good_callback(const event_t *event, void *user_data)
{
    k_sem_give(&processing_sem);  // 非阻塞
}
```

### 错误码说明

| 错误码 | 值 | 描述 |
|--------|-----|------|
| EVENT_OK | 0 | 成功 |
| EVENT_ERR_NO_MEM | -1 | 内存不足 |
| EVENT_ERR_QUEUE_FULL | -2 | 队列已满 |
| EVENT_ERR_QUEUE_EMPTY | -3 | 队列为空 |
| EVENT_ERR_INVALID_ARG | -4 | 无效参数 |
| EVENT_ERR_NOT_FOUND | -5 | 未找到 |
| EVENT_ERR_NO_SUBSCRIBER | -6 | 无订阅者 |
| EVENT_ERR_TIMEOUT | -7 | 超时 |
| EVENT_ERR_NOT_RUNNING | -8 | 事件系统/分发器未启动 |

---

## 附录

### 头文件包含

对外 API 统一放在 `include/zeplod/`，使用 `#include <zeplod/...>`；也可一次引入伞头文件 `zeplod_framework.h`。

```c
#include <zeplod/event_system.h>      // 核心 API
#include <zeplod/event_dispatcher.h>  // 调度器 API
/* event_queue.h 为框架内部实现，应用请使用 event_system / event_dispatcher API */
```

### 相关文档

- [模块系统详细使用说明.md](32-模块系统详细使用说明.md)
- [开发者入门指南.md](../00-入门/04-开发者入门指南.md)
- [Zephyr RTOS 官方文档](https://docs.zephyrproject.org/)
