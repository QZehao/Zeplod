> 语言: **中文** | [English](../../en/30-core-modules/38-thread-service-lifecycle.md)

# 线程服务生命周期约定

> 日期：2026-05-28  
> 范围：`event_dispatcher`、`ipc_service`、`data_bus` 分发线程  
> 关联：[27-核心架构重构路线图](../20-架构设计/27-核心架构重构路线图.md) P2、[tests/LIFECYCLE_CONTRACTS.md](../../../tests/LIFECYCLE_CONTRACTS.md)
...
| Join 失败 | 返回 `EVENT_ERR_TIMEOUT`，**不** abort | 返回 `-EIO`，**不** abort，保持 `STOPPING` 和资源以便重试 | 返回 `-EIO`，**不** abort，保持 `g_initialized` / `g_shutting_down` |
...
stateDiagram-v2
    ...
    STOPPING --> STOPPING: join 超时，稍后重试
...
- IPC join 超时后不 abort，保持 `STOPPING`、pending 和共享内存；线程退出后重试 `stop` 完成清理。
...
| ipc_service | 允许：保持 `STOPPING`，线程退出后再次 `stop` 完成 join 和资源清理 |
