...
Devicetree Overlay 的自动发现顺序、`app.overlay` 与 `boards/*.overlay` 的区别、主 RAM（`zephyr,sram`）与链接脚本、以及多块不连续内存时的典型做法，见 **[Zephyr设备树与内存配置手册.md](Zephyr设备树与内存配置手册.md)**。
...
- [Zephyr 设备树与内存配置手册](Zephyr设备树与内存配置手册.md)
- [项目 README](../README.md)
