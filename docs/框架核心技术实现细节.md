...
```c
// event_system.h:142-150
typedef struct {
    event_type_t     type;       // 事件类型 (0-255)
    event_priority_t priority;   // 事件优先级
    uint32_t         timestamp;  // 创建时间戳 (毫秒 uptime)
    uint32_t         source_id;  // 源模块 ID
    uint32_t         data_len;   // 数据长度
    void*            data;       // 数据指针
    bool             is_dynamic; // true: 动态分配，需释放
} event_t;
```

**设计考量**：
- `event_t` 本身是固定大小（约 28 字节），可以直接放入消息队列
- `data` 可以是静态数据（`is_dynamic = false`）或动态分配（`is_dynamic = true`）
- 静态数据零拷贝，动态数据由系统管理生命周期

#### 2.1.2 订阅者条目

```c
// event_system.h:169-175
typedef struct {
...
```c
// event_system.c:52-61
typedef struct {
    uint32_t           magic;                        // 魔数验证
    bool               initialized;                  // 初始化标志
    atomic_t           running;                      // 运行状态（原子变量）
...
event_t event = {
    ...
    .data = &sensor_data,
    .is_dynamic = false  // 静态数据
};
event_publish(&event);
// 发布后可立即重用 sensor_data
```
...
event_publish(event);
event_free(event);  // 只释放外壳

// 或使用便捷函数（自动管理）
event_publish_copy(type, priority, data, data_len);
```
