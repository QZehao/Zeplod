...
在 `prj.conf` 或 Kconfig 中配置：

```kconfig
# 事件队列配置
CONFIG_EVENT_QUEUE_SIZE=64          # 队列容量（事件数）

# 订阅者配置
CONFIG_EVENT_MAX_SUBSCRIBERS=16     # 每事件类型最大订阅者数

# 调度器配置
CONFIG_EVENT_DISPATCHER_STACK_SIZE=2048  # 调度器线程栈大小
CONFIG_EVENT_DISPATCHER_PRIORITY=5       # 调度器线程优先级
```
